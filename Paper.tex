...
I will conduct two brief Monte Carlo experiments to demonstrate that
...
The second \dgp\ is
\begin{align*}
  y_{t} &= 2.237 + 0.261 y_{t-1} + \gamma_{1} z_{t-1} + \gamma_{2}
...
the alternative.  In general, the statistics perform similarly well.

For the simulations with a single break, the new statistic modestly
higher power than \poscw\ original test.  For most parametrizations of
\dgp\ 1, the new test rejects over 90\% of the time, while Clark and
West's test rejects about 40\% of the time.  So mixing window
strategies can give a small power advantage when testing for
