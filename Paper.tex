...
\usepackage{amsfonts}
\usepackage{amsmath,amsthm,amssymb,graphicx,setspace,url,booktabs,tabularx,enumerate}
...
\author{Gray Calhoun\thanks{ Economics Department; Iowa State
    ...
    \texttt{http://www.econ.iastate.edu/$\sim$gcalhoun}.  This paper
    has benefited from my conversations with Helle Bunzel, Graham
    Elliott, Yu-Chin Hsu, Pablo Pincheira, and Allan Timmermann; thank
    you.  I'd also like to thank Amit Goyal for providing computer
    code and data for his 2008 \textsc{rfs} paper with Ivo Welch
    \citep{GoW:08}.} \\ Iowa State University}
...
\begin{abstract} 
  ...
  simultaneously while controlling family-wise error, and
...
beginning with \citet{DiM:95} and \citet{Wes:96} and continuing with
\citet{WeM:98}, \citet{Mcc:98,Mcc:00},
\citet{ClM:01,ClM:05-2,ClM:05,ClM:09b,ClM:11b,ClM:11}, \citet{CCS:01},
\citet{CoS:02,CoS:04,CoS:07}, \citet{Whi:00}, \citet{InK:04,InK:06},
\citet{Han:05}, \citet{Ros:05}, \citet{ClW:06,ClW:07}, \citet{Ana:07},
\citet{GiR:09,GiR:10}, \citet{HuW:10}, \citet{HLN:11}, \cite{InR:11},
\cite{Pin:11}, \cite{RoS:11,RoS:11b}, and \citet{Cal:11}, among
others.\footnote{For recent reviews of this literature and additional
  references, see \citet{McW:02}, \citet{CoS:06}, \citet{Wes:06},
  \citet{ClM:11c}, \citet{CoD:11}, and \citet{Gia:11}}

In a pair of papers, \citet{ClW:06,ClW:07} develop an \oos\ test of the
null hypothesis that a small benchmark model is correctly specified.
Their test compares the forecasting performance of a pair of nested
models, and the null hypothesis is that the innovations in the smaller
model form a martingale difference sequence.  This test procedure is
popular,\footnote{As of 12 September, 2011, \citet{ClW:06} has been
  cited by 28 papers indexed by the \textsc{isi} Web of Knowledge, and
  \citet{ClW:07} by 41. They have been cited by 132 and 172 papers
  indexed by Google Scholar respectively.} and one assumes that this
...
\oos\ tests for similar hypotheses \citep[among
...
define $\varepsilon_{t+1} = y_{t+1} - x_t'\beta_0$.  If the linear model
is correctly specified, so $\varepsilon_{t+1}$ is a martingale
difference sequence with respect to $\mathcal{F}_t = \sigma(y_{t+1},
x_t, v_t, y_t, x_{t-1}, v_{t-1}, \dots)$ then we can see immediately
that
\begin{equation}
  \label{eq:4}
...
\begin{thm}\label{res:1}
  ...
  $\mathcal{F}_t = \sigma(y_t, z_{t-1}, y_{t-1}, z_{t-2},\dots)$,
...
\citet{Han:05}, and \citet{ClM:11}, have emphasized statistics that
test families of hypotheses that control the probability any
hypothesis is rejected given that they all are true, known as
\textit{weak control of family-wise error} (\wfwe).  This paper
...
hypotheses, known as \textit{strong control of family-wise error}
...
slightly as below (see Part~\ref{it:1} of Theorem~\ref{res:2} as well as
Remark~\ref{rem:2}).
...
\begin{rem}\label{rem:2}
  The key difference between Parts~\ref{it:1} and~\ref{it:2} is that,
  in Part~\ref{it:2}, the forecast error is an \mds\ with respect to
  several individual series, but not necessarily with respect to the
  pooled information set generated by all of those series
  together.\footnote{It is straightforward to construct three series,
    $u_t$, $v_t$, and $w_t$, such that $w_t$ is an \mds\ with respect
    to $\sigma((u_t, w_t), (u_{t-1}, w_{t-1}),\dots)$ and
    $\sigma((v_t, w_t), (v_{t-1}, w_{t-1}),\dots)$ but not
    $\sigma((u_t, v_t, w_t), (u_{t-1}, v_{t-1}, w_{t-1}),\dots)$.
    This is essentially the null imposed in Part~\ref{it:2}.}  Using
  $b = 1$ would consistently estimate the marginal distribution of
  each $\tfrac1{\sigma_i} \bar{f}_i$ under this weaker null
  ...
  stronger hypotheses of Part~\ref{it:1}.  Fortunately, Part~\ref{it:1} seems to capture
  the goal of most empirical work, so researchers can make use of the
  considerable simplifications that occur in that setting.
\end{rem}

\begin{rem}
  The procedure described above is a refinement of the StepM for
  one-sided tests along the lines of \citepos{Han:05} \spa, but with
  better power.  Hansen's \spa\ is motivated by the observation that
  including models $i$ for which $\E \bar f_{i}$ is strongly negative
  reduces the test's power: the critical values are taken as the
  maximum over a larger number of models, but the $i$th model will
  almost never exceed the critical value.  Hansen suggests discarding
  models for which $\bar f_i$ is below a threshold implied by the Law
  of the Iterated Logarithm.  \citet{Cal:11e} suggests the threshold
  used in Theorem 2 and argues that it has better size and power
  properties.  See \citet{Cal:11e} for further motivation and
  explanation.
\end{rem}
...
\begin{rem}\label{rem:01}
  ...
  (Part~\ref{it:3}) easily accommodates this extension by taking each
  $\bar f_i$ and $\hat{\sigma}_{i}^{2}$ to be a random
  $q$-vector;\footnote{If a researcher uses multiple benchmark models,
    ...
    specified.} the first element of $\bar f_{i}$ compares the first
...
test, suggesting that the new statistic is a suitable replacement in
applied research.  The paper also presents a method for comparing the
benchmark model to several alternative models simultaneously and
improves block bootstrap procedures of \oos\ statistics.
...
\begin{proof}[Proof of Lemma~\ref{res:a3}]
  ...
  \begin{equation}\label{eq:2}
    ...
    \bar{f}(\beta_0))' \to^p 0.
  \end{equation}
  Proving~\eqref{eq:3} uses essentially the same argument
  as~\citet[Lemma A.3]{Mcc:00} and~\eqref{eq:2} essentially follows a
  streamlined version of the argument of~\ref{res:a5}, completing the
  proof.  The proofs for the moving and circular block bootstraps are
...
% LocalWords:  familywise prespecified CoD Gia pointwise misspecified
