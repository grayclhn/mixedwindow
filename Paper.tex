...
probability distribution equal sample means.%
\footnote{This equality is
  strictly true only for the stationary and circular block
  bootstraps.} %
The benchmark model still needs to be reestimated in
...
\begin{rem}\label{rem:01}
  ...
  model to outperform them both.%
\footnote{\citet{Pin:11} studies tests
  ...
  $q$-vector;%
\footnote{If a researcher uses multiple benchmark models,
    ...
    appropriate.} %
  the first element of $\bar f_{i}$, written as $\bar
