...
\begin{thm}\label{res:3}
  ...
  \item Let $\{y_t, z_t\}$ be an $L_2$-\textsl{\ned} process of size
    $-\frac12$ on the series $V_t$; $V_t$ is strong mixing of size
    $-\frac{r}{r-2}$ or uniform mixing of size $-\frac{r}{2r-2}$ with
    $r > 2$.\footnote{For uniform mixing processes, $r = 2$ is also
      allowed as long as $\sup_{\beta \in N} \psi_t(\beta)^2$ and
      $\sup_{\beta \in N} (\partial \psi_t(\beta) / \partial \beta)^2$
      are uniformly integrable; these terms are defined later in these
      assumptions.}  Moreover, the \ned-coefficients are uniformly
    bounded.
  \item The sequence $(y_1^{*}, z_t^*),\dots,(y_T^{*}, z_T^*)$ is
    ...
    distribution.  The (expected) block length $b$ satisfies $b \to
    \infty$ and $\frac{b}{P} \to 0$.
  ...
  \item Let $f_{t}(\beta) = f(y_{t}, z_{t}, \beta)$ be an $l$-vector;
    the asymptotic variance matrix of $\bar{f}(\beta_0)$ is uniformly
    positive definite.
  \item Define $\psi_t(\beta) = (f_t(\beta), h_t(\beta))$;
    $\psi_t(\beta)$ is covariance stationary and is continuously
    differentiable in an open neighborhood $N$ of $\beta_0$.  Also,
    $\E( \sup_{\beta \in N} |\psi_t(\beta)|)^r$ and $\E\sup_{\beta \in
      N} |\frac{\partial \psi_t(\beta)}{\partial \beta}|^2$ are
    uniformly bounded.
...
\begin{proof}[Proof of Lemma~\ref{res:a3}]
  ...
  \citet[Theorem~2]{Cal:11d}, so it suffices to prove that
  \[ \tfrac{ 1 }{ P } \var^* \Big( \sum_{t=R+1}^T \psi_t^*( \hat
  \beta_T ) \Big) - \tfrac{ 1 }{ P } \var \Big( \sum_{t=R+1}^T \psi_t(
  \beta_0 ) \Big) \to^p 0. \] I will prove this result for the Moving
  Blocks Bootstrap---the proof for other block bootstraps is similar
  and follows as in \cite{Cal:11d}.
  ...
  \end{equation*}
  by \citet[Theorem 2.2]{JoD:00}.  To see this, observe that 
  \begin{equation*}
    \su \hat\psi_s \hat\psi_t' 
    = \tfrac{1}{T} \sum_{s,t=1}^T \hat\psi_s \hat\psi_t' \;
      \big(1 - \tfrac{\lvert s - t \rvert}{b}\big)^+ + term.
  \end{equation*}
  The first term satisfies most of \citepos{JoD:00} Assumptions by
  construction and it suffices to prove their condition (2.9) holds,
  namely that
  \begin{equation}\label{eq:2}
    ...
    \Bigg\rVert_2 \to 0
  \end{equation}
  for all $i$, $j$, and $\xi$.  I will show that \eqref{eq:2} holds
  for the real part of the expression; the proof for the complex part
  is similar.

  For any fixed $\beta \in N$,
  \begin{equation*}
    \tfrac{1}{T} \sum_{t=1}^T \cos( \xi t / b )
    \Bigg( \frac{\partial \psi_{jt}(\beta)}{\partial \beta_i} - 
          \E \frac{\partial \psi_{jt}(\beta)}{\partial \beta_i} \Bigg) 
  \end{equation*}
  obeys an \lln\ (give reference and explanation -- you need to show
  that the derivatives are \ned\ as well.) and convergence in $L_2$
  follows from uniform integrability.

  TODO: add stochastic equicontinuity conditions.
\end{proof}
