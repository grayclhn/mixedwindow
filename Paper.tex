...
\begin{thm}\label{res:3}
  ...
  \item Define $\psi_t(\beta) = (f_t(\beta), h_t(\beta))$;
    ...
    differentiable with bounded second derivative in an open
    neighborhood $N$ of $\beta_0$.  Also, $\E \sup_{\beta \in N}
    |\psi_t(\beta)|^r$ and $\E\sup_{\beta \in N} |\frac{\partial
      \psi_t(\beta)}{\partial \beta'}|^r$ are uniformly bounded ($r$ is
    defined below).
  \item Both $\psi_t(\beta)$ and $\frac{\partial}{\partial \beta}
    \psi_t(\beta)$ are $L_2$-\textsl{\ned} of size $-\frac12$ on the
...
\begin{rem}
  ...
  condtions on $f_t(\beta)$ and $h_t(\beta)$: only their expectations
...
\begin{lema}\label{res:a3}
  ...
  \begin{equation}\label{eq:1}
    \pr\Big[\sup_x \Big| \pr^{*}\Big[ \tfrac{1}{\sqrt{P}} \sum_{t=R+1}^T \psi_{t}^{*}(\hat\beta_T)
    \leq x \Big] - \pr\Big[ \tfrac{1}{\sqrt{P}} \sum_{t=R+1}^T \psi_{t}(\beta_0)
...
\begin{proof}[Proof of Lemma~\ref{res:a3}]
  ...
  \[ \tfrac{ 1 }{ P } \var^* \Big( \sum_{t=R+1}^T \psi_t^*( \hat
  \beta_T ) \Big) - \tfrac{ 1 }{ P } \var \Big( \sum_{t=R+1}^T \psi_t(
  \beta_0 ) \Big) \to^p 0. \]
  ...
  probability to $(1/P) \var \sum_{t=R+1}^T \psi_t( \beta_0 )$
