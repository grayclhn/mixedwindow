...
\begin{equation}
  \label{eq:4}
  \oclt{t} \varepsilon_{t+1} (v_t - x_t'\beta_0)
\end{equation}
...
\begin{equation}
  \label{eq:5}
  \tfrac{2}{\sqrt{P}} \osum{t} \varepsilon_{t+1} (v_t -
  x_t'\beta_0) = \oclt{t} \Big[(y_{t+1} - 
...
\begin{thm}\label{res:1}
  ...
  \begin{align*}
    ...
    \hat{S}_{ff} &= \oavg{t} (f_t(\hat{\beta}_t) - \bar
    f)^2, \\
    \hat{S}_{fg} &= \oavg{t} (f_t(\hat{\beta}_t) -
    \bar{f})(g_t(\hat{\beta}_t) - \bar g)', &
    \hat{S}_{gg} &= \oavg{t} (g_t(\hat{\beta}_t) - \bar
    g)(g_t(\hat{\beta}_t) - \bar g)',
  \end{align*}
  $\Pi = 1 - \tfrac{R}{P} \log(1 + \tfrac{P}{R})$, $\bar{g} = \tfrac1T
  \osum{t} g_t(\hat{\beta}_t)$, $\X' = [x_1,\dots,x_T]$, and
  \begin{equation*}
    g_t(\beta) =
...
\begin{thm}\label{res:3}
  ...
  \item The estimator $\hat{\beta}_t$ of $\beta_0$ is estimated with a
    ...
    \begin{equation}
      H_{t} = \begin{cases} 
        \tfrac1t \sum_{s=1}^t h_{s}(\beta_0) & \text{recursive window} \\
        \tfrac1R \sum_{s=t-R+1}^t h_{s}(\beta_0) & \text{rolling window} \\
        \tfrac1R \sum_{s=1}^R h_{s}(\beta_0) & \text{fixed window} \\
      \end{cases}
    \end{equation}
    and $\E h_{s}(\beta_0) = 0$ for all $s$.
  \item Define $\psi_t(\beta) = (f_t(\beta), h_t(\beta))$;
    $\psi_t(\beta)$ is covariance stationary and twice continuously
    differentiable in an open neighborhood $N$ of $\beta_0$.  Also,
    $\E \sup_{\beta \in N} |\psi_t(\beta)|^r$ and $\E\sup_{\beta \in
      N} |\frac{\partial}{\partial \beta} \psi_t(\beta)|^r$ are
    uniformly bounded ($r$ is defined below), and there exists a
    sequence of random variables $m_t$ such that $\sup_{i,\beta \in N}
    |\tfrac{\partial^2}{\partial \beta \partial \beta'}
    \psi_{it}(\beta)| \leq m_t$ almost surely and $\E m_t^2$ is
    uniformly finite.
  \item Both $\psi_t(\beta)$ and $\frac{\partial}{\partial \beta}
    ...
    mixing of size $-\frac{r}{2r-2}$ with $r > 2$.\footnote{For
      ...
      integrable.}
  \item The sequence $\psi_1^{*},\dots,\psi_T^{*}$ is generated by the
  ...
  \item The bootstrapped estimator satisfies $\sup_t |\hat{B}_t^{*} -
    B^{*}| \to^p 0$, $B^{*} = B + o_p$ and $\frac1T \sum_{t=1}^T h_t^* = 0$ a.s.
...
\begin{rem}
  ...
  consistency of \hac\ estimators which requires differentiability of
...
\begin{rem}
  ...
  \bar{f}(\beta_0))$.  But it is clear that $\bar{f}(\hat{\beta}_T)$
  is the bootstrap analogue of $\E \bar{f}(\beta_0)$, the parameter of
...
\newcommand{\WesA}{\oclt{t} (\nabla
  f_t^*(\hat\beta_{T}) - F^*) B^* H_t^*}
\newcommand{\WesB}{\tfrac{1}{\sqrt{P}} F^* \osum{t} (B_t^* -
  B^*) H_t^*}
\newcommand{\WesC}{\oclt{t} (\nabla
  f_t^*(\hat\beta_{T}) - F^*) (B_t^* - B^*) H_t^*}
\begin{proof}[Proof of Theorem~\ref{res:3}]
  Expand $\E^{*} f_t^{*}(\hat{\beta}_t^{*})$ around $\hat{\beta}_{T}$
  to get
  \begin{align*}
    \sqrt{P} (\bar{f}^{*} - \bar{f}(\hat{\beta}_{T})) &= \oclt{t}
    f_t^{*}(\hat{\beta}_{T}) - \bar{f}(\hat{\beta}_{T}) +
    F^{*} B^{*} \oclt{t} H_t^* \\
    & \quad + \WesA + \WesB \\ & \quad + \WesC + \oclt{t} w_t^*
  \end{align*}
  where (as in \citealp{Wes:96}) the $i$th element of $w_t^*$ is
  \begin{equation*}
    w_{it} = \tfrac12 (\hat\beta_t^* - \hat\beta_T)'
    \Big[\tfrac{\partial^2}{\partial \beta \partial\beta'} 
    f_{it}^*(\tilde\beta_{it}^*) \Big]
    (\hat\beta_t^* - \hat\beta_T)
  \end{equation*}
  and each $\tilde\beta_{it}^*$ lies between $\hat\beta_t^*$ and
  $\hat\beta_T$.  The argument that $\oclt{t} w_t^* = o_{p^*}(1)$ is
  ...
  \end{gather}
  from Lemma~\ref{res:a4} and the result follows from Lemma~\ref{res:a3}.
\end{proof}
...
\begin{lema}\label{res:a4}
  Under the conditions of Theorem~\ref{res:3}, \eqref{eq:7},
  \eqref{eq:9}, and \eqref{eq:10} hold.
\end{lema}
\noindent For each of these results, I will present the proof for the
recursive window and the circular block bootstrap only; extensions to
the fixed and rolling window follow exactly as in \citet{WeM:98}.

\begin{proof}[Proof of Lemma~\ref{res:a4}]
We can write
\begin{equation*}
  \Big\lvert \WesA \Big\rvert \leq 
  \Big\lvert \oclt{t} (\nabla f_t^*(\hat\beta_T) - F^*) B^* \Big\rvert
  \sup_t | H_t^* |.
\end{equation*}
From Lemma~\ref{res:a2}, $\sup_t | H_t^* | \to^p 0$; $\oclt{t} (\nabla
f_t^*(\hat\beta_T) - F^*)$ obeys a \clt\ \citep{Cal:11d} and is
$O_{p^*}(1)$, establishing~\eqref{eq:7}.  The proofs of~\eqref{eq:9}
and~\eqref{eq:10} are similar.
\end{proof}

\begin{lema}\label{res:a3}
  Suppose the conditions of Theorem~\ref{res:3} hold and let
  \[\psi_t^{*}(\beta) = 
  \binom{f_t^{*}(\beta) - \bar{f}(\beta)}{h_t^{*}(\beta) - \bar h(\beta)}.\]
  Then
  \begin{equation}\label{eq:1}
    \pr\Big[\sup_x \Big| \pr^{*}\Big[ \oclt{t} \psi_{t}^{*}(\hat\beta_T)
    \leq x \Big] - \pr\Big[ \oclt{t} \psi_{t}(\beta_0)
...
\begin{proof}[Proof of Lemma~\ref{res:a3}]
  Note that both $\psi_t(\beta_0)$ and $\psi_t^*(\hat\beta_T)$ satisfy
  \clt s, (\citealp[Theorem~2]{Jon:97} and
  \citealp[Theorem~1]{Cal:11d}) so it suffices to prove that
  \[ \tfrac{ 1 }{ P } \var^* \Big( \osum{t} \psi_t^*( \hat
  \beta_T ) \Big) - \tfrac{ 1 }{ P } \var \Big( \osum{t} \psi_t(
  \beta_0 ) \Big) \to^p 0. \]
  I will prove this result for the Circular Block Bootstrap
  \citep{PoR:92}---the proof for other block bootstraps is similar and
  follows as in \cite{Cal:11d}.

  \newcommand{\su}{\tfrac{ 1 }{ b T } \sum_{ \tau = 1 }^{ T }%
  \sum_{ s, t \in I( \tau ) }}

  Since $\E^* \psi_t^*( \hat \beta_T ) = 0$ by construction, we have
  \begin{equation}\label{eq:3}
  \tfrac{ 1 }{ P } \var^* \Big( \sum_{ t = R + 1 }^T \psi_t^*( \hat \beta_T ) \Big)
  = \su \hat \psi_s( \hat \beta_T ) \hat \psi_t( \hat \beta_T )'
  \end{equation}
  almost surely, with
  \begin{equation*}
    I(\tau) =
  ...
  \begin{multline}\label{eq:6}
    \su \psi_s(\hat\beta_T) \psi_t(\hat\beta_T)'
    = \tfrac{1}{T} \sum_{s,t=1}^T \psi_s(\hat\beta_T) \psi_t(\hat\beta_T)' \;
      \big(1 - \tfrac{\lvert s - t \rvert}{b}\big)^+ \\
      + \tfrac{1}{bT} \sum_{s = 1}^{b - 1} \sum_{t = T - b + s}^{T}
        (\psi_s(\hat\beta_T) \psi_t(\hat\beta_T)' +
         \psi_t(\hat\beta_T) \psi_s(\hat\beta_T)') \;
        \big(1 - \tfrac{\lvert s - t + T\rvert}{b}\big)^+.
  \end{multline}

  The first term of \eqref{eq:6} can be shown to converge in
  probability to $(1/P) \var \osum{t} \psi_t( \beta_0 )$
  by Theorem 2.2 of \citet{JoD:00}.  Most of the necessary conditions
  hold by assumption and it suffices to prove their condition (2.9)
  holds, namely that
  \begin{equation}\label{eq:2}
    \sup_{\beta \in N} \Bigg\lVert \tfrac{1}{T} \sum_{t=1}^T e^{i \xi t/b} 
    \Bigg( \frac{\partial \psi_{jt}(\beta)}{\partial \beta_i} -
          \E \frac{\partial \psi_{jt}(\beta)}{\partial \beta_i} \Bigg) 
    \Bigg\rVert_2 \to 0
  \end{equation}
  for all $i$, $j$, and $\xi$.  Now, for any fixed $\beta \in N$,
  \begin{equation*}
  ...
  follows from the existence of the $r$th moment and uniform
  convergence is a consequence of the bounded second derivative of
  $\psi_t$.
  ...
  \begin{multline}
    \Big| \tfrac{1}{bT} \sum_{s = 1}^{b - 1} \sum_{t = T - b + s}^{T}
        \psi_{is}(\hat\beta_T) \psi_{jt}(\hat\beta_T)
        \big(1 - \tfrac{\lvert s - t + T\rvert}{b}\big)^+ \Big| \leq\\
     \tfrac{1}{T} \sum_{s = 1}^{b - 1} |\psi_{is}(\hat\beta_T)|
         \times\tfrac{1}{b} \sum_{t = T - b + 1}^{T} | \psi_{jt}(\hat\beta_T) |
  \end{multline}
  almost surely.  Now, for any positive $\delta$,
  \begin{equation*}
    \pr \Bigg[ \tfrac{1}{T} \sum_{s = 1}^{b - 1} | \psi_{is}( \hat \beta_T ) |
              > \delta \Bigg] \leq 
  ...
  Both terms converge to zero by assumption, so $\tfrac{1}{T} \sum_{s
    = 1}^{b - 1} |\psi_{is}(\hat\beta_T)| = o_p(1)$.  A similar
  argument shows that $\tfrac{1}{b} \sum_{t = T - b + 1}^{T} |
  \psi_{jt}(\hat\beta_T) | = O_p(1)$, completing the proof.
\end{proof}
...
% LocalWords:  outperformance Hmisc Har nondifferentiable bT jt
%  LocalWords:  stationarity mixingale mixingales texextra
