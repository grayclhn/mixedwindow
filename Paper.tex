...
\begin{rem}
  ...
  \bar{f}(\beta_0))$.  But it is clear that $\bar{f}(\btrue[*])$
...
\newcommand{\WesA}[1][]{\oclt{t} (\nabla
  f_t^{#1}(\btrue[#1]) - F^{#1}) B^{#1} H_t^{#1}}
\newcommand{\WesB}[1][]{\tfrac{1}{\sqrt{P}} F^{#1} \osum{t} (B_t^{#1} -
  B^{#1}) H_t^{#1}}
\newcommand{\WesC}[1][]{\oclt{t} (\nabla
  f_t^{#1}(\btrue[#1]) - F^{#1}) (B_t^{#1} - B^{#1}) H_t^{#1}}
\begin{proof}[Proof of Theorem~\ref{res:3}]
  Expand $f_t^{*}(\hat{\beta}_t^{*})$ around $\btrue[*]$
  to get
  \begin{align*}
    \sqrt{P} \big(\bar{f}^{*} - \bar{f}(\btrue[*])\big) &= \oclt{t}
    \big(f_t^{*}(\btrue[*]) - \bar{f}(\btrue[*])\big) +
    F^{*} B^{*} \oclt{t} H_t^* \\
...
\begin{lema}\label{res:a2}
  ...
  \item $P^a \sup_t | \hat{\beta}_{t} - \beta_{0} | \to^{p} 0$ and
    ...
    \to^{p^{*}} 0$.
  \item $\tfrac{1}{\sqrt{P}} \sum_{t=R+1}^T
    \big(\tfrac{\partial}{\partial \beta} f_t(\beta_0) - F\big) =
    O_{p}(1)$ and $\tfrac{1}{\sqrt{P}} \sum_{t=R+1}^T
    \big(\tfrac{\partial}{\partial \beta} f_t^*(\hat\beta_{T+1}) -
    F^*\big) = O_{p^*}(1)$.
  \end{enumerate}
\end{lema}
...
\begin{proof}[Proof of Lemma~\ref{res:a2}]
  \begin{enumerate}
  \item The process $\tfrac{1}{\sqrt{T}} h_{s}(\beta_0)$ satisfies
    \citepos[Theorem 3.1]{JoD:00b} functional \clt.  So
    \begin{equation}
      P^a \sup_t \Big| \tfrac1t \sum_{s=1}^t h_{s}(\beta_0) \Big| =
      P^a \sup_{\gamma \in [0,1]} \Big| \tfrac{1}{\lfloor \gamma
        T\rfloor} \sum_{s=1}^{\lfloor \gamma T \rfloor} h_{s}(\beta_0) \Big| \to^{p} 0
    \end{equation}
    with the convergence following from the continuous mapping
    theorem.  For the bootstrapped average, \citepos{Cal:11d}
    Theorem~2 ensures that
    \begin{equation}
      \tfrac{1}{\sqrt{\gamma
          T}} \sum_{s=1}^{\lfloor \gamma T \rfloor} h_{s}^*(\hat\beta_{T+1}) 
      = O_{p^*}(\Omega^*(\hat\beta_{T+1})^{1/2})
    \end{equation}
    where $\Omega^*(\beta) = \var^* h_t^*(\beta)$.  Since
    $\hat\beta_{T+1} \to^p \beta_0$, it suffices to show that
  ...
  \item We have
    \begin{multline}
      ...
      \tfrac{P^a}{t} \sum_{s=1}^t h_{s}(\beta_0) \Big| + \sup_{t} \Big|
      B \tfrac{P^a}{t} \sum_{s=1}^t h_{s}(\beta_0) \Big|
    \end{multline}
  ...
  \item $\tfrac{1}{\sqrt{P}} \sum_{t=R+1}^T
    \big(\tfrac{\partial}{\partial \beta} f_t(\beta_0) - F\big)$ obeys
    \citepos{Jon:97} \clt, so the result is trivial.  Also,
   \begin{equation}
     \Big\{\tfrac{1}{\sqrt{P}} \sum_{t=R+1}^T 
     \big(\tfrac{\partial}{\partial \beta} f_t^*(\hat\beta_{T+1}) -
     F^*\big)\Big\} = O_{p^*}(\Omega^*(\hat\beta_{T+1})^{1/2}) = O_p(1)
    \end{equation}
    as in the proof of part 1, where now $\Omega^*(\beta) = \var^*
    \tfrac{\partial}{\partial \beta} f_t^*(\beta)$.
  \end{enumerate}
\end{proof}
...
\begin{equation*}
  ...
  \Big\lvert \oclt{t} (\nabla f_t^*(\hat\beta_{T+1}) - F^*) B^* \Big\rvert
  \sup_t | H_t^* |.
\end{equation*}
From Lemma~\ref{res:a2}, $\sup_t | H_t^* | \to^p 0$ and $\oclt{t}
(\nabla f_t^*(\hat\beta_{T+1}) - F^*) = O_p(1)$,
establishing~\eqref{eq:7}.  The proofs of~\eqref{eq:9}, \eqref{eq:10},
and \eqref{eq:11}--\eqref{eq:13} are similar.
\end{proof}
...
\begin{lema}\label{res:a3}
  ...
  \begin{equation}\label{eq:1}
    \pr\Big[\sup_x \Big| \pr^{*}\Big[ \oclt{t} \psi_{t}^{*}(\hat\beta_{T+1})
    \leq x \Big] - \pr\Big[ \oclt{t} \psi_{t}(\beta_0)
    \leq x \Big] \Big| > \epsilon \Big] \to 0
  \end{equation}
  for all positive $\epsilon$, where the inequality holds element-by-element.
\end{lema}

\begin{proof}[Proof of Lemma~\ref{res:a3}]
  Note that both $\psi_t(\btrue)$ and $\psi_t^*(\btrue[*])$ satisfy
  \clt s, (\citealp[Theorem~2]{Jon:97} and
  \citealp[Theorem~1]{Cal:11d}) so it suffices to prove that
  \[ \tfrac{ 1 }{ P } \var^* \Big( \osum{t} \psi_t^*(\btrue[*])) \Big)
  - \tfrac{ 1 }{ P } \var \Big( \osum{t} \psi_t(\btrue) \Big) \to^p
  ...
  Since $\E^* \psi_t^*( \hat \beta_{T+1} ) = 0$ by construction, we have
  \begin{equation}\label{eq:3}
  \tfrac{ 1 }{ P } \var^* \Big( \sum_{ t = R + 1 }^T \psi_t^*( \hat \beta_{T+1} ) \Big)
  = \su \hat \psi_s( \hat \beta_{T+1} ) \hat \psi_t( \hat \beta_{T+1} )'
  \end{equation}
  almost surely, with
  \begin{equation*}
    I(\tau) =
  ...
  \begin{multline}\label{eq:6}
    \su \psi_s(\hat\beta_{T+1}) \psi_t(\hat\beta_{T+1})'
    = \tfrac{1}{T} \sum_{s,t=1}^T \psi_s(\hat\beta_{T+1}) \psi_t(\hat\beta_{T+1})' \;
      \big(1 - \tfrac{\lvert s - t \rvert}{b}\big)^+ \\
      + \tfrac{1}{bT} \sum_{s = 1}^{b - 1} \sum_{t = T - b + s}^{T}
        (\psi_s(\hat\beta_{T+1}) \psi_t(\hat\beta_{T+1})' +
         \psi_t(\hat\beta_{T+1}) \psi_s(\hat\beta_{T+1})') \;
        \big(1 - \tfrac{\lvert s - t + T\rvert}{b}\big)^+.
  \end{multline}

  The first term of \eqref{eq:6} can be shown to converge in
  probability to $(1/P) \var \osum{t} \psi_t(\btrue)$
  ...
  \begin{multline*}
    \Big| \tfrac{1}{bT} \sum_{s = 1}^{b - 1} \sum_{t = T - b + s}^{T}
        \psi_{ks}(\hat\beta_{T+1}) \psi_{jt}(\hat\beta_{T+1})
        \big(1 - \tfrac{\lvert s - t + T\rvert}{b}\big)^+ \Big| \leq\\
     \tfrac{1}{b T} \sum_{s = 1}^{b - 1} |\psi_{ks}(\hat\beta_{T+1})|
         \sum_{t = T - b + 1}^{T} | \psi_{jt}(\hat\beta_{T+1}) |
  \end{multline*}
  almost surely.  For any positive $\delta$,
  \begin{equation*}
    \pr \Bigg[ \tfrac{1}{T} \sum_{s = 1}^{b - 1} | \psi_{is}( \hat \beta_{T+1} ) |
              > \delta \Bigg] \leq 
  ...
  Both terms converge to zero by assumption, so $\tfrac{1}{T} \sum_{s
    = 1}^{b - 1} |\psi_{is}(\hat\beta_{T+1})| = o_p(1)$.  A similar
  argument shows that $\tfrac{1}{b} \sum_{t = T - b + 1}^{T} |
  \psi_{jt}(\hat\beta_{T+1}) | = O_p(1)$, completing the proof.
\end{proof}
...
\begin{lema}\label{res:a5}
  ...
  \begin{equation}
    \oclt{t}
    (f_{t}(\hat{\beta}_{t}) - \E f_{t}(\btrue)) \to^d N(0, \sigma^2),
  \end{equation}
  with 
  \begin{align}
  ...
  variance of~$\hat\beta_{T+1}$.
\end{lema}

\begin{proof}[Proof of Theorem~\ref{res:3}]
  As in \citet{Wes:96} and \citet{WeM:98} (and as in the proof of
  Theorem~\ref{res:3}), expand $f_t(\hat{\beta}_t^{*})$ around
  $\btrue$ to get
  \begin{align*}
    \sqrt{P} \big(\bar{f} - \bar{f}(\btrue)\big) &= \oclt{t}
    \big(f_t(\btrue) - \E f_t(\btrue)\big) +
    F B \oclt{t} H_t \\
    & \quad + \WesA + \WesB \\ & \quad + \WesC + \oclt{t} w_t
  \end{align*}
  ...
  $f_t(\btrue)$ and $H_t$ both satistfy \clt s, this completes the
  proof.
\end{proof}
