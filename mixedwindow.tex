...
This paper proposes a modified version of their statistic and shows that it is
...
alternative models. Section~\ref{sec:4} concludes. Proofs of our
results have been put in a separate Appendix along with some
supporting results \citep{Cal:15}.
...
straightforward to use Theorem~\ref{res:1} and Lemma~\ref{lem:2} to
derive an appropriate critical value similar to~\citet{Whi:00}.
...
\begin{gather*}
  ...
  \intertext{and}
  \gh_{it} = \Bigg[\oavg{s} (4 x_s'\bh_s - 2 y_{s+1} - 2 \yh_{i,s+1}) x_s'\Bigg]\,
            \Bigg[\tfrac{1}{T-1} \sum_{s=1}^{T-1} x_s x_s'\Bigg]^{-1} x_t \eph_{t+1}
\end{gather*}
respectively. Call this estimate $\hat V$ and let $\hat c$ denote the
$1 - \alpha$ quantile of the distribution of $\max_i \hat W_i$, with
$\hat W \sim N(0, \hat V)$.  Then
\begin{equation*}
  \limsup_{T \to \infty} \Pr\Big[\max_{i=1,\dots,\nmod} \sqrt{P} \fb_i / \sigmah_{1i} > \hat c\Big] \leq \alpha,
\end{equation*}
...
remain significant at 10\% using this critical value. Using this
critical value, none of the predictors are significant at 10\%, which
