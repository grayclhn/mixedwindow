...
\footnote{Exceptions are \citepos{StW:07}
  \allcaps{IMA}(1,1) and \allcaps{UC}-\allcaps{SV} models of inflation.} %
but the
...
allowing for multiple alternative models, as in \cite{Whi:00} and \cite{HuW:10}. This
modification is crucial if we want the statistic to be empirically
...
denoted $\yh_{j,t+1}$, for $j = 1,\dots,J$, and is estimated with a
...
forecasts are otherwise unrestricted, so even well-behaved predictors
...
Our last assumption is on the asymptotic variance of $\fb^*$.
\begin{asmp}\label{a4}%
  The asymptotic variance of $\fb^*$ is positive definite.
\end{asmp}
...
Note that Theorem~\ref{res:1} does not require any of the models to be
correctly specified. Asymptotic normality follows from the weak
dependence of the underlying series and from the design of the test
statistic. Typically these statistics are used to test the null
hypothesis that the benchmark model is correctly specified, which
implies that $\fh_t - \E f_t^*$ is an \mds\ as discussed in
Section~\ref{sec:1a}. This is especially appealing in our framework,
since the benchmark can be theoretically motivated so the \mds\ null
can be a test of forecaster rationality.
But that null hypothesis only affects the
estimator of $\Sigma$ (see Lemma~\ref{lem:2}); Theorem~\ref{res:1}
continues to hold under alternatives that satisfy Assumptions
\ref{a1} -- \ref{a4}.

Moreover, Theorem~\ref{res:1} can be used with a \hac\ estimator of
$\Sigma$ when a researcher wants to test the weaker hypothesis that
$\E \fb^* = 0$ but the benchmark may be misspecified. This approach
could be interpreted as an encompassing test as in \citet{HLN:98}, and
would test whether the alternative models contain additional
information not present in the benchmark model. In either case (\mds\
or non-\mds), this test is designed to have power against the
``encompassing'' alternative. The specific research question should
then determine whether the test should also reject if there are other
violations of rationality.

\newcommand{\K}[1]{K((#1)/w)}
For the next results, define the variables $\gh_t = \Fh \Bh
x_t(y_{t+1} - x_t'\bh_t)$, $\gb = \oavg{t} \gh_t$, and
\begin{align*}
  \Fh &= \Big\{\tfrac{2}{P}\sum_{s=R}^{T-1} x_s (x_s'\beta - \yh_{1,s+1}) \Big\}' &
  &\text{and}&
  \Bh &= \Big(\tfrac1P \sum_{s=R+1}^{T-1} x_s x_s' \Big)^{-1}
\end{align*}
Then define the \hac\ covariance estimator $\Sigmah_1$ and the \mds\
covariance estimator $\Sigmah_2$ as
\begin{align*}
...
\end{align*}
where $K$ is a kernel that satisfies standard assumptions.

\begin{asmp}\label{a5}
  Fill in assumptions on $\K$.
\end{asmp}

\begin{lem}\label{lem:2}\input{mixedwindow_lem2}\end{lem}


Put a last result here about one-sided testing.
