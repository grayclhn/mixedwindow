...
\author{Gray Calhoun\thanks{ Economics Department; Iowa State
    ...
    Michael McCracken, Pablo Pincheira, Allan Timmermann, Ken West, Stephane
...
with a recursive window and the alternative models with a fixed length
...
high power against alternative \dgp s that cause the benchmark model
...
This paper shows that a modified version of their statistic is
...
as well. As \citet{ClM:11b} have recently pointed out,
...
null. (\citealp{ClM:11b}, call this scenario ``overlapping models.'')

The next section presents the intuition and theory for our new statistic.
Section~\ref{sec:2} presents
simulations that compare our pairwise \oos\ test to \poscw\ original
statistics and Section~\ref{sec:3} demonstrates the use of our statistic
...
some supporting results \citep{Cal:14b}.
...
their 2007 paper, where $\btrue$ is unknown and estimated with length-$R$
...
specific assumptions for the more general case.
...
\begin{equation*}
  \Fs_t = \sigma(y_t, x_{t+1}, z_{t+1}, y_{t-1}, x_t, z_t,\dots).
\end{equation*}
...
\begin{gather*}
  \gh_t = \Bigg[\oavg{s} (4 x_s'\bh_s - 2 y_{s+1} - 2 \yh_{s+1}) x_s'\Bigg]\,
          \Bigg[\tfrac{1}{T-1} \sum_{s=1}^{T-1} x_s x_s'\Bigg]^{-1} x_t \eph_{t+1}
  \intertext{and}
  g_t = \E\Big[(4 x_t'\beta - 2 y_{t+1} - 2 \yh_{t+1}) x_t'\Big] \, (\E x_t x_t')^{-1} x_t \ep_{t+1}
\end{gather*}
and the \oos\ averages $\fb = \osum{t} \fh_t/P$, $\fb^* = \osum{t} f_t/P$,
$\fb = \osum{t} \gh_t/P$, and $\gb^* = \osum{t} g_t/P$.
...
at the beginning of the Section. This is especially appealing in our
...
model will have smaller \mse\ than the benchmark model unless
$e_{0,t+1}$ and $e_{0,t+1} - e_{1,t+1}$ are uncorrelated, implying that
the alternative uses information not in the benchmark.  As the
discussion at the beginning of this section shows, the test in
Theorem~\ref{res:1} is equivalent to a test of zero correlation
between $e_{0,t+1}$ and $e_{0,t+1} - e_{1,t+1}$.
