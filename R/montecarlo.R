...
bench <- function(d) lm(y ~ 1, data = d)
alter <- function(d) lm(y ~ zlag, data = d)

r <- mcdesign[1,]

for (r in rows(mcdesign)) {
  insert(mcdata) <- data.frame(r, t(replicate(nsim, {
    d <- with(r, rdgp(R + P, gam))
    ## run clark and west's test
    c(pOld = oos.t(bench, alter, d, r$R, method = "ClW:07", conf.level = .9)$p.value,
      pNew = oos.t(bench, alter, d, r$R, method = "Cal:11", conf.level = .9)$p.value)
