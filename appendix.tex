...
additional terms:
\begin{equation*}
  F_t(\beta) = (4 x_t'\beta - 2 y_{t+1} - 2 \yh_{t+1}) x_t',
\end{equation*}
...
\begin{proof}
  ...
  \item Our assumptions ensure that $x_t \ep_{t+1}$ is $L_2$-mixingale
    ...
    constants and coefficients. Note that $t^{a-1} x_t \ep_{t+1}$ is
    also an $L_2$-mixingale array with constants $t^{a-1} c_s$ and
    coefficients $\zeta_k$, since
    \begin{align*}
      \| \E_{t-k} x_t \ep_{t+1} t^{a-1} \| &= t^{a-1} \| \E_{t-k} x_t \ep_{t+1} \| \\
      &\leq (t^{a-1} c_t) \zeta_k
    \end{align*}
    and
    \begin{align*}
      \| x_t \ep_{t+1} t^{a-1} - \E_{t+k} x_t \ep_{t+1} t^{a-1} \| &= t^{a-1} \|  x_t \ep_{t+1} - \E_{t+k} x_t \ep_{t+1} \| \\
      &\leq (t^{a-1} c_t) \zeta_{k+1}.
    \end{align*}

    Let $\delta$ be a positive number less than $1/2 - \alpha$, so
    \begin{align*}
      \E\Bigg[\omaxb{t} \Big|(t-1)^{a-1} & \sum_{s=1}^{t-1} x_s \ep_{s+1} \Big|^2\Bigg] \\
      &\leq (R'-1)^{-\delta} \E\Bigg[\omaxb{t} \Big| \sum_{s=1}^{t-1} x_s \ep_{s+1} (s-1)^{a-1+\delta/2} \Big|^2\Bigg] \\
      &\leq (R'-1)^{-\delta} O(1) \sum_{s=1}^{T-1} (s-1)^{2a - 2 + \delta}
    \end{align*}
    ...
    \infty$ and $(R'-1)^{-\delta} \to 0$, completing the proof.

  \item As in Part 1, $x_t \ep_{t+1} t^{a-1}$ is an $L_2$-mixingale of size
    $-1/2$ and we can use \citepos{Mcl:75} maximal inequality to get
    \begin{align*}
    ...
    \end{align*}
    which again converges to a finite limit.
